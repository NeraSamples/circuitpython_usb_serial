...
label = storage.getmount("/").label  # CIRCUITPY or else
theID = board.board_id
theSN = "".join([f"{x:02x}" for x in microcontroller.cpu.uid])
MY_ID = f"{theID}:{label}:{theSN}"
...
while True:
    ...
    if usb_cdc.data.in_waiting > 0:
        ...
        # received string, interpret and respond
        if isinstance(data, str):

            # receive hello, respond with temperature
            if data == "Hello":
                print("Hi !")
                data_out["temperature"] = microcontroller.cpu.temperature

        # received json, interpret
